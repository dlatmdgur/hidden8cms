...
@section('content')
    ...
    <div class="right_col" role="main">
        <div class="">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item">가방 지급/회수 로그</li>
            </ol>
            ...
            <div class="row">
                <div class="col-md-12 col-sm-12  ">
                    <div class="x_panel">
                        ...
                        <div class="x_panel">
                            <div class="x_title">
                                <h2>가방 지급/회수 로그<small>(일별)</small></h2>
                                <div class="clearfix"></div>
                            </div>
                            <div class="x_content">
                                <table class="table-bordered info-table" style="width: 100%">
                                    <tr>
                                        <th width="15%">처리구분</th>
                                        <td><label class="col-form-label col-md-2 col-sm-2 label-align">
                        ...
                        <div class="x_panel">
                            <div class="x_title">
                                <h2>가방 지급/회수 로그<small>(개별)</small></h2>
                                <div class="clearfix"></div>
                            </div>
                            <div class="x_content">
                                <table class="table-bordered info-table" style="width: 100%">
		...
		<div class="modal-dialog modal-md modal-xl">
			<div class="modal-content" style="overflow-y:auto; margin-top:50px">
				<!-- <form name="frmApply" method="post" onsubmit="return false;" method-transfer="async" class="m-0"> -->
					...
					<div class="modal-header"><h5 class="modal-title"><b>가방 지급/회수 로그 상세 : </b><strong class="date-space"></strong></h5></div>
    ...
    <script>
        var actionNames = $.parseJSON('{!! json_encode(Helper::adminActions()) !!}');
        var presentNames = $.parseJSON(@json(json_encode(\App\Model\Tables\Item::getItemNames())));
        ...
        $(document).ready(function () {
                ...
                buttons: [
                    {
                        extend: 'excelHtml5',
                        title: function() {
                            return '운영로그_가방지급회수로그(일별)_' + $("#search_start_date1").val().replace(/-/gi, '') +'-'+$("#search_end_date1").val().replace(/-/gi, '');
                        },
                        exportOptions: {
                            format: {
                                body: function(data, row, column, node) {
										if ([5].indexOf(column) > -1)
                                            return '-';
                                        else if(column === 4)
                ...
                buttons: [
                    {
                        extend: 'excelHtml5',
                        title: function() {
                            return '운영로그_가방지급회수로그(개별)_' + $("#search_start_date2").val().replace(/-/gi, '') +'-'+$("#search_end_date2").val().replace(/-/gi, '');
                        },
                        ...
                        if (adminLogs.length > 0) {
                            ...
                            $(adminLogs).each(function(index, log) {
                                  ...
                                  let tr = $(`<tr>
                                    ...
                                    <td> ${presentNames[log.presentType]} </td>
                                    <td> ${log.changeAmount} </td>
                                    <td> <button type="button" data-date="${log.logDate}" data-type="${log.presentType}" data-action-type="${log.actionType}" data-toggle="modal" data-target="#daily-detail" class="btn btn-secondary">Detail</buton></td>
                                    </tr>`);
            ...
            $('#daily-detail').on('show.bs.modal', function(e) {
                const emb = $(e.relatedTarget);
                const emf = $(e.currentTarget);
                const date = emb.data('date');
                const mb = emf.find('.modal-content .modal-body');

                mb.empty();
                emf.find('.modal-content .modal-header .date-space').text(date);


                let q = {'date': date, 'type': emb.data('type'), 'action_type': emb.data('action-type') };
                q = Object.entries(q).map(e => e.join('=')).join('&');

                $.get({
                    url: ['/log/postsLogs/detail', q].join('?'),
                    dataType: 'json',
                    success: function(d) {
                        mb.append(d.html);
                    }
