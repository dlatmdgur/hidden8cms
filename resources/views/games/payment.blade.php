    ...
    <div class="right_col" role="main">
        <div class="">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{ route('games.payment') }}">결제내역 조회</a></li>
            </ol>
            ...
            <div class="row">
                <div class="col-md-12 col-sm-12  ">
                            ...
                            <div class="col-md-12">
                                <div class="x_panel">
                                    <div class="x_content" style="padding-bottom: 0;">
                                        ...
                                        <div class="row  col-md-12 col-sm-12 item form-group">
                    ...
                    <div class="x_panel">
                        <div class="x_title">
                            <h2>결제 내역 조회<small>(대상 : <span id="target"></span>)</small></h2>
                            <div class="clearfix"></div>
                        </div>
                        <div class="x_content">
                ...
                buttons: [
                    {
                        extend: 'excelHtml5',
                        title: function() {
                            return '결제내역_' + $('#nickname').val() + '_' + $("#search_start_date").val().replace(/-/gi, '') +'-'+$("#search_end_date").val().replace(/-/gi, '');
                        }
                    }
                ],
            });
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data.userInfo !== undefined && data.userInfo !== null) {
                            ...
                            $('#target').text(data.userInfo.nickname);
                        } else {
                            $('#target').text('전체');
                        }
                        ...
                        if (billingInfo.length > 0) {
                            let no = 1;
                            $(billingInfo).each(function(index, log) {

                                let tr = $('<tr>' +
                                    '<td>' + no++ + '</td>' +
                                    '<td>' + log.data.user_seq + '</td>' +
                                    '<td>' + log.data.nickname + '</td>' +
                                    '<td>' + log.data.account + '</td>' +
                                    '<td>' + log.data.reg_date + '</td>' +
                                    '<td>' + log.data.update_date + '</td>' +
                                    '<td>' + log.marketType + '</td>' +
                                    '<td>' + log.data.order_id + '</td>' +
                                    '<td>' + log.data.product_name + '</td>' +
                                    '<td>' + numberFormat(log.data.price) + '</td>' +
                                    '</tr>');
