...
@section('content')
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            all_game_table = $('#all_game_table').DataTable({
                aaSorting: [],
                bSort: false,
                pageLength: 1000,
                ...
                buttons: [
                    {
                        extend: 'excelHtml5',
                        title: function() {
                            return '게임상세_' + $('#userSeq').val() + '_' + $("#startDate").val().replace(/-/gi, '') +'-'+$("#endDate").val().replace(/-/gi, '');
                        }
                    }
                ],
                rowsGroup: [0,1,2,3]
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    for (let i = 0; i < allGameLogs.length; i++) {
                        ...
                        if (log.gameType === 3) {
                            ...
                            if (parseInt(log.betPlayer) > 0) {
                                baccaratBet += '<div style="margin:0; padding: 0; border-bottom: 1px solid #dee2e6; width:100%; height: 40px;line-height: 40px;">' +
                                    ...
                                    numberToKorean(log.betPlayer)+'</span>'+
                                    '</div>';
                            }
                            if (parseInt(log.betBanker) > 0) {
                                baccaratBet += '<div style="margin:0; padding: 0; border-bottom: 1px solid #dee2e6; width:100%; height: 40px;line-height: 40px;">' +
                                    '<span style="display: inline-block; width: 70px; margin: 0; padding: 0 10px 0 0; border-right: 1px solid #dee2e6; text-align: right;">' +
                                    'Banker(x1)</span>' +
                                    '<span style="display: inline-block; width: 80px; margin: 0; padding: 0 0 0 10px; text-align: right;">' +
                                    numberToKorean(log.betBanker)+'</span>'+
                                    '</div>';
                            }
                            if (parseInt(log.betPPair) > 0) {
                                baccaratBet += '<div style="margin:0; padding: 0; border-bottom: 1px solid #dee2e6; width:100%; height: 40px;line-height: 40px;">' +
                                    '<span style="display: inline-block; width: 70px; margin: 0; padding: 0 10px 0 0; border-right: 1px solid #dee2e6; text-align: right;">' +
                                    'P.Pair(x11)</span>' +
                                    '<span style="display: inline-block; width: 80px; margin: 0; padding: 0 0 0 10px; text-align: right;">' +
                                    numberToKorean(log.betPPair)+'</span>'+
                                    '</div>';
                            }
                            if (parseInt(log.betTie) > 0) {
                                baccaratBet += '<div style="margin:0; padding: 0; border-bottom: 1px solid #dee2e6; width:100%; height: 40px;line-height: 40px;">' +
                                    '<span style="display: inline-block; width: 70px; margin: 0; padding: 0 10px 0 0; border-right: 1px solid #dee2e6; text-align: right;">' +
                                    'Tie(x8)</span>' +
                                    '<span style="display: inline-block; width: 80px; margin: 0; padding: 0 0 0 10px; text-align: right;">' +
                                    numberToKorean(log.betTie)+'</span>'+
                                    '</div>';
                            }
                            if (parseInt(log.betBPair) > 0) {
                                baccaratBet += '<div style="margin:0; padding: 0; border-bottom: 1px solid #dee2e6; width:100%; height: 40px;line-height: 40px;">' +
                                    '<span style="display: inline-block; width: 70px; margin: 0; padding: 0 10px 0 0; border-right: 1px solid #dee2e6; text-align: right;">' +
                                    'B.Pair(x11)</span>' +
                                    '<span style="display: inline-block; width: 80px; margin: 0; padding: 0 0 0 10px; text-align: right;">' +
                                    numberToKorean(log.betBPair)+'</span>'+
                                    '</div>';
                            }
                            // combine
                            row += '<td></td>'+
                                '<td>' + ( (log.leave === 1)? '방나감' : '-' ) + '</td>' +
                                '<td width="180">' +
                                '<div class="card_list">' +
                        ...
                        //rows += row;
                        all_game_table.row.add($(row));
                        all_game_table.rowsgroup.updateNextDraw();
                    }
