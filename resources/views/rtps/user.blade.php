	...
	<div class="right_col" role="main">
		<div class="">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item">유저별 RTP 설정</li>
			</ol>
...
@include('rtps.nav')
						...
						<form name="frm" method="get" action="{{ route('rtp.user') }}">
...
@if (!empty($rtps) && count($rtps) > 0)
	@foreach ($rtps AS $key => $row)
										...
										<tr class="text-center" data-user_seq="{{ $row->user_seq }}" data-nickname="{{ $row->nickname }}">
											<td>{{ $row->user_seq }}</td>
											<td>{{ $row->nickname }}</td>
											<td class="text-right pl-3 pr-3">{{ number_format($row->play) }}</td>
											<td class="text-right pl-3 pr-3">{{ number_format($row->bet) }}</td>
											<td class="text-right pl-3 pr-3">{{ number_format($row->win) }}</td>
											<td class="text-right pl-3 pr-3" data-type="avg">{{ number_format($row->real_rtp, 2) }}</td>
											<td>
	...
	<div id="apply" class="modal fade" tabindex="-1">
		<div class="modal-dialog modal-md">
			<div class="modal-content">
				<form name="frmApply" method="post" action="{{ Route('rtp.user.set') }}" onsubmit="return false;" method-transfer="async" class="m-0">
					<input type="hidden" name="user_seq" value="" />
					<input type="hidden" name="rtp" value="" />

					<div class="modal-header"><h5 class="modal-title"><b>USER RTP SETTING</b></h5></div>

					<div class="modal-body">
						<p class="m-1"><b>적용 대상 : </b><span id="nickname" class="ml-3">닉네임 ( 12345 )</span></p>
