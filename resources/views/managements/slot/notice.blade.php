...
@section('content')
    ...
    <div class="right_col" role="main">
        <div class="">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{ route('managements.slot.notice') }}">운영관리</a></li>
                <li class="breadcrumb-item">슬롯 공지</li>
            </ol>
                ...
                <div class="col-md-12 col-sm-12  ">
                    <div class="x_panel">
                        ...
                        <div class="x_content">
                            ...
                            <table class="table table-bordered info-table" id="notice_table" style="width:100%;">
                                <thead>
                                <tr>
                        ...
                        <div class="x_panel editor-box">
                            <div class="x_title">
                                <h2>공지사항 등록 / 수정 [ 현재 모드 : <span id="cur_mode">등록</span> ] <small id="cur_article"></small></h2>
                                <div class="clearfix"></div>
                            </div>
                            <div class="x_content">
    ...
    <script>
        ...
        const noticeCategories = $.parseJSON('{!! json_encode(Helper::noticeCategory()) !!}');
        const csStatus = $.parseJSON('{!! json_encode(Helper::csStatus()) !!}');

        $(document).ready(function() {
            let notice_table = $('#notice_table').DataTable({
                aaSorting: [],
                bSort: false,
                pageLength: 10,
            ...
            let updateArticle = function(params) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '/management/slot/updateNotice',
                    data: {
                        id: params.id,
                        title: params.title,
            ...
            $('#btn-preview').off('click').on('click', function () {
                ...
                $.ajax({
                    type: 'GET',
                    url: '/management/slot/previewNotice',
                    data: {
                        from: 'notice'
                    },
                    dataType: 'json',
                    success: function(data) {
                        ...
                        showPreview('notice', '/preview/slot/notice/NoticeList.html');
                    },
                    error: function(data) {
                        if (data.status === 419) {
                            alert('세션이 만료되었습니다.');
            ...
            $('#btn-publish').off('click').on('click', function () {
                ...
                $.ajax({
                    type: 'GET',
                    url: '/management/slot/publishNotice',
                    data: {
                        from: 'notice'
                    },
                    dataType: 'json',
                    success: function(data) {
                        ...
                        showPublish('notice', '/publish/slot/notice/NoticeList.html');
                    },
                    error: function(data) {
                        if (data.status === 419) {
                            alert('세션이 만료되었습니다.');
        ...
        function loadList(params) {
            ...
            $.ajax({
                type: 'POST',
                url: '/management/slot/listNotice',
                data: {
        ...
        $(document).on('click', '.btn-edit-article', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    // update edit panel
                    let article = data.noticeArticle;
        ...
        function drawTable(data) {
            ...
            if (noticeList.length > 0) {
                ...
                $(noticeList).each(function(index, notice) {
                    ...
                    let tr = $('<tr id="tr_'+notice.id+'">' +
                        '<td><input type="checkbox" class="article_check" name="notice_id" value="'+ notice.id +'" /></td>' +
                        '<td>' + (++no) + '</td>' +
                        '<td><a class="change_order" data-direction="up" data-id="'+ notice.id +'" data-order="'+ notice.order +'"><i class="fa fa-arrow-up"></i></a></td>' +
                        '<td><a class="change_order" data-direction="down" data-id="'+ notice.id +'" data-order="'+ notice.order +'"><i class="fa fa-arrow-down"></i></a></td>' +
                        '<td>' + gameTypes[notice.game_type].name + '</td>' +
                        '<td>' + noticeCategories[notice.category].name + '</td>' +
                        '<td>' + notice.title + '</td>' +
                        '<td>' + notice.os.toUpperCase() + '</td>' +
                        '<td>' + csStatus[notice.status].name + '</td>' +
                        '<td>' + notice.admin_name + '</td>' +
                        '<td>' + getFormattedDatetime(notice.update_date) + '</td>' +
                        '<td><button class="btn-sm btn-success btn-edit-article" data-id="'+notice.id+'">수정</button> '+
                        '<button class="btn-sm btn-danger btn-delete-article" data-id="'+notice.id+'">삭제</button></td>' +
                        '</tr>');
                    notice_table.row.add(tr);
                });
            }
            notice_table.draw();
        }
        ...
        function changeMode() {
            $('#notice_table tr').children('td, th').css('background', '#FFF');
            $('#cur_article').text(' ');
            $('#edit_mode').val('new');
            reset();
        }
        ...
        function checkedIds()
        {
            let ids = $("#notice_table input:checkbox:checked").map(function(){
                if ($(this).val() !== 'all') {
                    return $(this).val();
                }
            }).get();
