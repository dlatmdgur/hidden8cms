...
@if (count($data) > 0)
	...
	@foreach ($data AS $idx1 => $row)
		...
		@endif
											...
											<td class="text-right pr-1">{{ number_format($row['rate'], 2) }}%</td>
											<td class="text-right pr-1">{{ number_format($row['bet_total']) }}</td>
											<td class="text-right pr-1">{{ number_format($row['fee_total']) }}</td>
											<td class="text-right pr-1">{{ number_format($row['win_total']) }}</td>

		@foreach ($row['pay'] AS $val)
											<td class="text-right pr-1 text-break">{{ number_format($val[0]) }}<span class="text-info ml-1" style="font-size: 11px;">({{ number_format($val[1], 2) }})</span></td>
		@endforeach

											<td class="text-right pr-1">{{ number_format($row['game_played']) }}</td>
											<td class="text-right pr-1">{{ number_format($row['game_played_f']) }}</td>

<?php /*
		@foreach ($row['avg_rate'] AS $val)
											<td class="text-right pr-1 text-break">
			@if (is_array($val))
												{{ @number_format($val[0]) }} <span class="text-info" style="font-size: 11px;">({{ @number_format($val[1], 2) }})%</span>
			@else
												{{ @number_format($val) }}
			@endif
											</td>
