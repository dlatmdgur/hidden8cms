...
class CsFaq extends BaseModel
{
    protected $connection = 'mysql';
    protected $table = 'cms_game.cs_faq';
...
//        }
    ...
    {
        $sql = "SELECT max(`order`) as maxOrder from cms_game.cs_faq;";
        return collect(DB::connection('mysql')->select($sql))->first();
    }

    public static function updateOrder($article)
    {
        $sql = " UPDATE cms_game.cs_faq SET `order` = ? WHERE id = ? ";
    ...
    {
        $idSet = implode(',', $ids);
        $sql = "UPDATE cms_game.cs_faq SET ".
                " `".$colName."` = '". $colValue . "' ".
